...
import spacy  # Correctly import the spacy module
...
def extract_entities_spacy(article):
    ...
    nlp = spacy.load('en_core_web_sm')  # Use spacy to load the model
    # Process the article to extract entities
    doc = nlp(article)
    # Create a set of entities with their labels
...
def compare_entities(nltk_entities, spacy_entities):
    ...
    # Find entities only found by NLTK
    nltk_only_entities = nltk_entities - spacy_entities
    # Find entities only found by SpaCy
